 ...
 * assigned tasks - fires even if the Cloud Functions scheduled sweep
 * hasn't run yet or the device has no connectivity at the exact due time.
 * Cross-user "you were assigned a task" alerts are handled server-side via
 * FCM (see /functions), since this worker can only see local state.
 */
@HiltWorker
